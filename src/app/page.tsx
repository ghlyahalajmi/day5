...
const FEATURES = [
  {
    // Each feature carries one hue of the palette, in order.
    chip: "bg-lilac-300",
    title: "Log every cup",
...
export default function LandingPage() {
  return (
    <>
      ...
      <header className="aurora-glow grain relative bg-plum-900 text-mist-100">
        <nav className="relative mx-auto flex w-full max-w-6xl items-center justify-between gap-3 px-5 py-5 sm:px-8">
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            <Link
              href="/login"
              className="rounded-full px-3 py-2 text-sm font-medium whitespace-nowrap text-mist-200 transition-colors hover:bg-plum-700 hover:text-mist-50 sm:px-4"
            >
              Log in
            </Link>
            <Link
        ...
        <div className="relative mx-auto grid w-full max-w-6xl items-center gap-14 px-5 pt-12 pb-20 sm:px-8 sm:pt-16 sm:pb-28 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <h1 className="mt-6 font-display text-5xl leading-[0.95] font-semibold tracking-tight text-mist-50 sm:text-6xl lg:text-7xl">
              A Gahwa
              <br />
              <span className="palette-text">Log</span>
            </h1>

            <p className="mt-5 max-w-md font-display text-xl text-mist-200 italic sm:text-2xl">
              Your private log of great gahwa.
            </p>

            <div className="mt-6 flex items-center gap-3" aria-hidden="true">
              <span className="sadu-diamond bg-azure-300" />
              <span className="h-px flex-1 max-w-[10rem] bg-gradient-to-r from-lilac-500/70 to-transparent" />
            </div>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Link
                href="/login"
                className="inline-flex items-center justify-center rounded-full bg-azure-300 px-7 py-3.5 text-base font-semibold text-plum-900 shadow-lg shadow-black/20 transition-colors hover:bg-azure-400"
              >
                Log in
              </Link>
              <Link
                href="/signup"
                className="inline-flex items-center justify-center rounded-full border border-mist-300/40 px-7 py-3.5 text-base font-semibold text-mist-100 transition-colors hover:bg-plum-700"
              >
                Create account
              </Link>
            </div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2">
              <GuestButton className="text-sm font-medium text-mist-300 underline underline-offset-4 transition-colors hover:text-sky-200 disabled:opacity-60" />
              <Link
                href="/discover"
                className="text-sm font-medium text-mist-300 underline underline-offset-4 transition-colors hover:text-sky-200"
              >
                Where to drink gahwa in Kuwait
              </Link>
            </div>
            <p className="mt-1 text-xs text-mist-300/70">
          ...
          <div className="relative mx-auto w-full max-w-sm lg:max-w-none">
            <div className="arch absolute -inset-3 border border-azure-300/25" aria-hidden="true" />
            <div className="arch relative overflow-hidden bg-mist-50 shadow-2xl shadow-black/30">
              <div className="px-6 pt-20 pb-6">
                ...
                <div className="mt-4 space-y-3">
                  {SAMPLE.map((item) => (
                    <article
                      key={item.place}
                      className="rounded-2xl border border-mist-200 bg-white p-4 shadow-sm"
                    >
      ...
      <main className="dawn-glow w-full">
        <div className="mx-auto w-full max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-3">
            {FEATURES.map((feature, index) => (
              <section
                key={feature.title}
                className="lift relative rounded-3xl border border-mist-200 bg-white p-6 shadow-sm"
              >
                <span
                  className={`arch-sm flex h-11 w-9 items-end justify-center pb-1.5 font-display text-base font-semibold text-plum-900 ${feature.chip}`}
                >
                  {index + 1}
                </span>
      ...
      <footer className="mt-auto bg-plum-900 text-mist-300">
        ...
        <div className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8">
          <div className="palette-rule h-[3px] w-24 rounded-full" aria-hidden="true" />
